...
class WC_PB_Cart {
	...
	*/
	...
	public function cart_item_remove( $cart_item_key, $cart ) {
		...
		if ( wc_pb_is_bundle_container_cart_item( $cart->removed_cart_contents[ $cart_item_key ] ) ) {
		...
		} elseif ( ! isset( $_POST[ 'update-bundle' ] ) && wc_pb_maybe_is_bundled_cart_item( $cart->removed_cart_contents[ $cart_item_key ] ) ) {
			...
			// Update the stamp.
			$container_key = wc_pb_get_bundled_cart_item_container( $cart_item, false, true );
			WC()->cart->cart_contents[ $container_key ][ 'stamp' ] = $stamp;
			foreach ( wc_pb_get_bundled_cart_items( WC()->cart->cart_contents[ $container_key ], false, true ) as $child_key ) {
				WC()->cart->cart_contents[ $child_key ][ 'stamp' ] = $stamp;
			}

			// Update the bundled items array.
			WC()->cart->cart_contents[ $container_key ][ 'bundled_items' ] = array_diff( WC()->cart->cart_contents[ $container_key ][ 'bundled_items' ], array( $cart_item_key ) );
